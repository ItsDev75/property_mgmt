import { CommercialProperty } from '../models/commercial_property.model';
...
export class CommercialPropertyService {
  ...
  async getPropertyById(id: number) {
    try {
      const property = await CommercialProperty.findByPk(id);
      if (!property) {
  ...
  async updateProperty(id: number, data: any) {
    try {
      const [updated] = await CommercialProperty.update(data, {
        where: { id },
      });
      if (!updated) {
        throw new Error('Commercial property not found');
  ...
  async deleteProperty(id: number) {
    try {
      const deleted = await CommercialProperty.destroy({
        where: { id },
      });
      if (!deleted) {
        throw new Error('Commercial property not found');
  ...
  async listProperties(page: number = 1, limit: number = 10, filters: any = {}) {
    try {
      ...
      const { count, rows } = await CommercialProperty.findAndCountAll({
        where: filters,
        limit,
        offset,
      });
